...
async function getVideoHongbao() {
    ...
    var uuid = getUuid();
    var config_text = await req.get("https://storage.360buyimg.com/babel/00395792/873571/production/dev/config.js", {
        qs: {
            t: Date.now()
        }
    });
    var { custom: { answerTime, adIds }, activityId, activityIdKey, pageId } = eval(`(${/\{[\s\S]*\}/.exec(config_text.replace("window.location.href", "1"))[0]})`);
    let now = new Date();
    let h = now.getHours();
    if (h < answerTime[0]) {
        if (h < 10) {
        ...
        await tools_1.delayRun(`${h}:00:20`);
    }
    ...
    let shopId = advertInfos.find((item) => item.groupId === "03303162")
    ...
    let { data: { answerStatusCode, hbStockPercent, userVo } } = JSON.parse(state_text);
    ...
    let answer = advertInfos.find((item) => item.groupId === "03303165" || item.groupName === "题目/选项/答案");
